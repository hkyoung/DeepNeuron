...
def compare_features(features_results, features):
    ...
    for feature in features:
        #RMS
        # comp = [diff_lists(features_results[feature][i], features_results[feature][i + 1]) for i in range(0, len(features_results[feature]) - 2, 2)]
        #Difference of means
        comp = [np.mean(features_results[feature][i]) - np.mean(features_results[feature][i + 1]) for i in range(0, len(features_results[feature]) - 2, 2)]
        print(feature, len(comp))
        np.savetxt('./mainen/pairwise/4p/comps/mcomp_' + feature + '.csv', comp, delimiter = ' ')
        ...
        plt.xlabel('Difference of mean of random pairs (' + units_dict[feature] + ')')
        plt.title(feature)
        plt.plot()
        plt.savefig('./mainen/pairwise/4p/plots/mcomp_hist_' + feature)
